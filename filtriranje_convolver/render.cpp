...
const unsigned int gInputChannel = 1;


bool setup(BelaContext *context, void *userData)
{
	std::ifstream file("w_eq_real_LMS.csv");
    std::string line;
...
void render(BelaContext *context, void *userData)
{
	...
	for(unsigned int n = 0; n < context->audioFrames; n++) {
		for(unsigned int channel = 0; channel < NUM_OF_SPEAKERS; ++channel)
			audioWrite(context, n, channel, 2*outBuf[n]);
		//	audioWrite(context, n, channel, 0.8*inBuf[n]);
	}
